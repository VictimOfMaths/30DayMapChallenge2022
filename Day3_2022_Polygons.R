...
England2122 <- England %>% 
  filter(Stadium %in% c("Emirates Stadium", "Villa Park", "Gtech Community Stadium", "Dean Court",
                        "Falmer Stadium", "Stamford Bridge", "Selhurst Park", "Goodison Park", 
                        "Craven Cottage", "Elland Road", "Brentford Community Stadium",
  ...
  mutate(Club=if_else(is.na(Club), Club[Stadium=="Gtech Community Stadium"],
                          Club),
         Capacity=if_else(is.na(Capacity), Capacity[Stadium=="Gtech Community Stadium"],
                          Capacity)) %>% 
  filter(Stadium!="Gtech Community Stadium") %>% 
  group_by(Stadium) %>% 
  slice(1) %>% 
  st_as_sf()%>% 
...
Scotland2122 <- Scotland %>% 
  filter(Stadium %in% c("Pittodrie Stadium", "Celtic Park", "Tannadice Park",
...
agg_png("Day3_2022_Polygons.png", units="in", width=8, height=9, res=700, background="lightcyan")
ggplot()+
...
agg_png("Day3_2022_PolygonsLondon.png", units="in", width=8, height=9, res=700, background="lightcyan")
ggplot()+
  geom_sf(data=outline, aes(geometry=geometry), fill="White", colour=NA)+
  geom_sf(data=voronoi, aes(geometry=geometry, fill=rownames(voronoi)), colour="Black",
          size=0.2, show.legend=FALSE)+
